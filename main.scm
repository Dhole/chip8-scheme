...
(define (chip8-op-drw ch8 pos-x pos-y n)
  ...
  (let* ((pos-x (remainder pos-x 64))
        ...
        (do ((i 0 (add1 i)))
          ((= i n))
          (let* ((byte (vector-ref mem (remainder (+ ch8-i i) mem-size)))
                 (y (+ pos-y i))
                 (a (shru8 byte shift))
...
(define (chip8-exec ch8 w0 w1)
  (case (bitwise-and w0 #xf0)
    ...
    ((#xE0) (case w1
              ((#x9E) (chip8-op-skp ch8 (chip8-v-lo-nib ch8 w0)))
...
(define (key-map sym)
  (case sym
    ((n-1) (arithmetic-shift 1 #x1))
    ((n-2) (arithmetic-shift 1 #x2))
    ((n-3) (arithmetic-shift 1 #x3))
    ((n-4) (arithmetic-shift 1 #xC))

    ((q)   (arithmetic-shift 1 #x4))
    ((w)   (arithmetic-shift 1 #x5))
    ((e)   (arithmetic-shift 1 #x6))
    ((r)   (arithmetic-shift 1 #xD))

    ((a)   (arithmetic-shift 1 #x7))
    ((s)   (arithmetic-shift 1 #x8))
    ((d)   (arithmetic-shift 1 #x9))
    ((f)   (arithmetic-shift 1 #xE))

    ((z)   (arithmetic-shift 1 #xA))
    ((x)   (arithmetic-shift 1 #x0))
    ((c)   (arithmetic-shift 1 #xB))
    ((v)   (arithmetic-shift 1 #xF))
...
(define (main-loop renderer texture ch8)
  (let ((done #f)
        (timestamp (sdl2:get-ticks)))
    (while (not done)
           (let ((ev (sdl2:poll-event!)))
             (when ev
               (case (sdl2:event-type ev)
                 ((quit) (set! done #t))
                 ((key-down) (let ((sym (sdl2:keyboard-event-sym ev)))
                                  (case sym
                                    ((escape)
                                     (set! done #t))
                                  )
                                  (chip8-keypad-set! ch8 (bitwise-ior (chip8-keypad ch8) (key-map sym)))
                             ))
                 ((key-up) (let ((sym (sdl2:keyboard-event-sym ev)))
                                (chip8-keypad-set! ch8 (bitwise-and (chip8-keypad ch8) (bitwise-not (key-map sym))))
                           ))
               )
             )
          )
          ...
          (let* ((now (sdl2:get-ticks))
                 (sleep-dur (truncate (- (/ frame-time 1000) (- now timestamp)))))
            (when (> sleep-dur 0)
               (sdl2:delay! sleep-dur))
            (set! timestamp (sdl2:get-ticks)))
    )
  )
)
